...
The configured remote path defaults to `edgewarden`. Edgewarden creates that directory with `MKCOL` when necessary and stores one self-contained ZIP for each successful backup:
...
Each ZIP contains the database snapshot and, when file inclusion is enabled, the referenced attachment and file-Send objects:
...
`manifest.json` records the backup format, table counts, blob sizes, and full blob hashes. `db.json` contains the allowlisted D1 rows. Attachment and Send files remain encrypted vault material, but the archive also contains account, configuration, and audit metadata. Treat the entire ZIP as sensitive and do not publish or share it as if it were an opaque encrypted file.
