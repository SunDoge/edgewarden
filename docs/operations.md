...
Both deployment configurations install an hourly Cron Trigger at minute 17. The handler runs due backups and bounded cleanup: expired sessions, challenges and download tokens are removed; old login/auth-request records are pruned; expired invitations are closed; and deleted attachments, ciphers, Sends, organizations, or accounts are removed together with their R2/KV objects. User-facing deletion first creates a D1 tombstone. Account deletion also bans login and revokes refresh tokens immediately; organization deletion immediately hides membership and vault access. Garbage collection proceeds from attachment objects to Cipher/Send rows and only then removes the owning organization or account. An R2/KV failure therefore leaves the complete ownership chain queued naturally for the next Cron instead of creating an unreachable blob or deleting an owner prematurely. Each invocation processes at most 100 attachments, 100 ciphers, and 100 Sends so a large backlog is drained over subsequent runs without exceeding a Worker invocation.

Audit history and secret retention are deliberately separate. Audit events are append-only through the API and are retained permanently by default; there is no bulk-clear endpoint. An operator may explicitly configure finite retention for privacy or D1 capacity requirements. After the restore window, GC still removes encrypted payloads and blob contents. Do not retain vault ciphertext or attachment content merely to extend audit history.
