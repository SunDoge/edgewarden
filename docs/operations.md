...
Both deployment configurations install an hourly Cron Trigger at minute 17. The handler runs due backups and bounded cleanup: expired sessions, challenges and download tokens are removed; old login/auth-request records are pruned; expired invitations are closed; and deleted attachments, ciphers, or Sends are removed together with their R2/KV objects. User-facing deletion first creates a D1 tombstone. Garbage collection deletes every external object before removing that tombstone; an R2/KV failure therefore leaves the item queued naturally for the next Cron instead of creating an unreachable blob. Each invocation processes at most 100 attachments, 100 ciphers, and 100 Sends so a large backlog is drained over subsequent runs without exceeding a Worker invocation.
