...
- One product version in the root package manifest, updated by cog using npm version. Internal packages are private and do not have separate versions.
...
- 根 package.json 保存唯一产品版本，由 cog 调用 npm version 更新；内部包均为私有包，不维护独立版本。
