...
export const CreateInviteSchema = v.object({
	masterPasswordHash: v.pipe(v.string(), v.minLength(1)),
	expiresInHours: v.optional(v.pipe(v.number(), v.integer(), v.minValue(1), v.maxValue(720)), 168),
});
...
export const AuditLogSettingsSchema = v.pipe(v.object({
	retentionDays: v.nullable(v.optional(v.picklist([7, 30, 90, 180, 365]))),
	maxEntries: v.nullable(v.optional(v.pipe(v.number(), v.integer(), v.minValue(100), v.maxValue(1_000_000)))),
}), v.check((value) => !(value.retentionDays && value.maxEntries), "Choose retention days or maximum entries, not both"));
