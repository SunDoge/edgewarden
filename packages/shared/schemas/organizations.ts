...
export const UpdateOrganizationSchema = v.object({ name: v.pipe(v.string(), v.minLength(1), v.maxLength(128)) });
export const DeleteOrganizationSchema = v.object({ masterPasswordHash: v.pipe(v.string(), v.minLength(1)) });
...
export const InviteOrganizationMemberSchema = v.object({
	...
	collections: v.optional(v.array(v.object({
...
export const OrganizationInviteeQuerySchema = v.object({ email: v.pipe(v.string(), v.email()) });
...
export type InviteOrganizationMemberInput = v.InferOutput<typeof InviteOrganizationMemberSchema>;
