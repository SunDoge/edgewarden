...
async function hmac(secret: string, message: string): Promise<string> {
	...
	return base64(new Uint8Array(await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(message))));
}

describe("Yubico OTP verification", () => {
	test("round-trips JSON configuration and safely rejects malformed values", () => {
		const stored = serializeYubikeyConfig({ keys: ["ccccccbbbbbb"], nfc: true });
		assert.deepEqual(parseYubikeyConfig(stored), { keys: ["ccccccbbbbbb"], nfc: true });
		assert.deepEqual(parseYubikeyConfig("not-json"), { keys: [], nfc: false });
		assert.deepEqual(parseYubikeyConfig('{"keys":"invalid","nfc":true}'), { keys: [], nfc: false });
		assert.deepEqual(parseYubikeyConfig(JSON.stringify({ keys: ["  CCCCCCBBBBBB  ", 3], nfc: false })), { keys: ["ccccccbbbbbb"], nfc: false });
	});
	...
	test("normalizes modhex and rejects public IDs without an OTP", () => {
		...
		assert.equal(yubicoPublicId("ccccccbbbbbbccccccbbbbbbccccccbb"), "ccccccbbbbbb");
	});
	...
	test("checks nonce, OTP, status and signed response", async () => {
		...
		const fetcher: typeof fetch = async (input) => {
			...
			const values = new URLSearchParams({ nonce, otp, status: "OK", timestamp: "2026-08-11T00:00:00Z" });
			const canonical = [...values.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([key, value]) => `${key}=${value}`).join("&");
			return new Response(`${canonical.split("&").join("\n")}\nh=${await hmac(secretKey, canonical)}\n`);
		};
		assert.equal(await verifyYubicoOtp(otp, { clientId: "123", secretKey }, { fetcher }), true);
		assert.equal(await verifyYubicoOtp(otp, { clientId: "123", secretKey }, { fetcher: async () => new Response(`otp=${otp}\nstatus=OK\nh=forged\n`) }), false);
	});
});
