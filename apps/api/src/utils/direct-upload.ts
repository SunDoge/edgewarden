...
export function buildDirectUploadUrl(
	request: Request,
	path: string,
	token: string,
): string {
...
export function getSafeJwtSecret(env: CloudflareBindings): string | null {
...
export async function parseDirectUploadPayload(
	...
	if (contentType.includes("multipart/form-data")) {
		...
		if (expectedFileName && file.name !== expectedFileName) {
			return errorResponse(
				fileNameMismatchMessage || "File name does not match.",
				400,
			);
		}
		if (
	...
	const uploadSize =
		declaredSize ?? (expectedSize && expectedSize > 0 ? expectedSize : null);
	if (uploadSize === null) {
