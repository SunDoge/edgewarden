...
async function encryptionKey(secret: string): Promise<CryptoKey> {
	const material = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(`edgewarden:yubico-config:v1:${secret}`));
...
export async function loadYubicoCredentials(db: Kysely<DB>, env: CloudflareBindings): Promise<YubicoCredentials | null> {
	const envClientId = String((env as any).YUBICO_CLIENT_ID ?? "").trim();
	const envSecret = String((env as any).YUBICO_SECRET_KEY ?? "").trim();
	if (envClientId && envSecret) return { clientId: envClientId, secretKey: envSecret };
	const row = await db.selectFrom("config").select("value").where("key", "=", CONFIG_KEY).executeTakeFirst();
	if (!row) return null;
	try {
