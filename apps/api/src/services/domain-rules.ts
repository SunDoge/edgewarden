...
function normalizeGlobalDomain(
	entry: RawGlobalDomain,
): GlobalEquivalentDomain | null {
...
const bitwardenGlobalDomains = normalizeGlobalDomains(
	bitwardenGlobalDomainsRaw,
);
...
export function normalizeEquivalentDomains(input: unknown): string[][] {
	...
	for (const group of input) {
		...
		const domains = Array.from(
			new Set(group.map(normalizeDomain).filter(Boolean)),
		);
...
export function normalizeCustomEquivalentDomains(
	input: unknown,
): CustomEquivalentDomain[] {
	if (!Array.isArray(input)) return [];
	...
	for (const [index, item] of input.entries()) {
		...
		const domains = normalizeEquivalentDomains([
...
export function customRulesToActiveEquivalentDomains(
