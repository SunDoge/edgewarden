...
const env = { TURNSTILE_SECRET_KEY: "test-secret" } as unknown as CloudflareBindings;
...
describe("Turnstile verification", () => {
	test("is optional when no secret is configured", async () => {
		...
		assert.equal(await verifyTurnstileToken(disabled, ""), true);
	});

	test("sends the token and client IP to Siteverify and requires the login action", async () => {
		let submitted: Record<string, unknown> = {};
		const ok = await verifyTurnstileToken(env, "valid-token", "203.0.113.10", async (_input, init) => {
		...
		assert.equal(await verifyTurnstileToken(env, "valid-token", undefined, async () => Response.json({ success: true, action: "register" })), false);
	});

	test("fails closed for missing tokens and Siteverify failures", async () => {
		assert.equal(await verifyTurnstileToken(env, ""), false);
		assert.equal(await verifyTurnstileToken(env, "token", undefined, async () => { throw new Error("offline"); }), false);
		assert.equal(await verifyTurnstileToken(env, "token", undefined, async () => Response.json({ success: false })), false);
	});
});
