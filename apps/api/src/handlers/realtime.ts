...
export const createRealtimeConnectionTicket = factory.createHandlers(async (c) => {
	if (!(c.env as any).REALTIME) return errorResponse("Realtime service is not configured", 503);
	const user = c.get("user");
	const token = await createRealtimeTicket(user.id, user.security_stamp, c.env.JWT_SECRET);
...
export const connectRealtime = factory.createHandlers(async (c) => {
	...
	const namespace = (c.env as any).REALTIME as DurableObjectNamespace | undefined;
	if (!namespace) return errorResponse("Realtime service is not configured", 503);
	...
	if (!user || user.security_stamp !== claims.sstamp || user.status !== "active") {
