...
import { DisableTotpSchema, RecoverTwoFactorSchema, TotpSetupSchema } from "../schemas/two-factor";
...
import { checkAccountRateLimit, checkIpRateLimit } from "../middleware/rate-limit";
...
export const listTwoFactor = factory.createHandlers(async (c) => {
	...
	if (isTotpEnabled(c.get("user").totp_secret)) providers.push({ enabled: true, type: 0, object: "twoFactorProvider" });
	if (userYubicoPublicIds(c.get("user") as any).length) providers.push({ enabled: true, type: 3, object: "twoFactorProvider" });
	if (await webauthnDb.countAccountPasskeyCredentialsByUserId(c.get("db"), c.get("user").id, "twoFactor")) providers.push({ enabled: true, type: 7, object: "twoFactorProvider" });
...
export const getAuthenticator = factory.createHandlers(async (c) => {
	const secret = c.get("user").totp_secret ?? randomBase32Secret();
	return c.json({
		key: secret,
		enabled: isTotpEnabled(c.get("user").totp_secret),
	...
	async (c) => {
		...
		await executeBatch(c.get("dbDialect"), [
			db
				...
				.set({
					totp_secret: key,
					totp_recovery_code: generateRecoveryCode(),
...
export const getRecoveryCode = factory.createHandlers(async (c) =>
	c.json({
		code: c.get("user").totp_recovery_code ?? null,
...
export const recoverTwoFactor = factory.createHandlers(
	...
	async (c) => {
		...
		if (!(await checkIpRateLimit(c)) || !(await checkAccountRateLimit(c, email))) {
		...
		const validPassword = user
			? await verifyPassword(body.masterPasswordHash, user.master_password_hash, user.email)
			: false;
		const validCode = user?.totp_recovery_code
			? constantTimeEqual(suppliedCode, normalizeRecoveryCode(user.totp_recovery_code))
			: false;
		...
		const ts = now();
		await executeBatch(c.get("dbDialect"), [
			db.updateTable("users").set({ totp_secret: null, totp_recovery_code: null, yubikey_config: serializeYubikeyConfig({ keys: [], nfc: false }), security_stamp: crypto.randomUUID(), updated_at: ts }).where("id", "=", user.id).compile(),
			db.deleteFrom("refresh_tokens").where("user_id", "=", user.id).compile(),
			db.deleteFrom("webauthn_credentials").where("user_id", "=", user.id).where("purpose", "=", "twoFactor").compile(),
		]);
		invalidateUserCache(user.id);
		await safeWriteAuditEvent(db, { actorUserId: user.id, action: "account.two_factor.recovered", category: "auth", level: "warning", targetType: "user", targetId: user.id, metadata: { ...auditRequestMetadata(c.req.raw), email } });
