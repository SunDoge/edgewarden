<script lang="ts">
  ...
  const canManage = $derived(["owner", "admin", "manager"].includes(organization.role));
</script>
          ...
          {#if canManage}
            <div class="flex shrink-0 gap-1">
              <Button
                ...
                aria-label={`重命名集合 ${collection.name}`}><Pencil data-icon /></Button
              >
              <Button
                variant="ghost"
                size="icon-sm"
                onclick={() => onremove(collection)}
                aria-label={`删除集合 ${collection.name}`}><Trash2 data-icon /></Button
              >
            </div>
          {/if}
        </div>
