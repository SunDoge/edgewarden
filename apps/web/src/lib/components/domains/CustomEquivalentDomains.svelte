...
import { CheckSquare, Edit, Globe, Plus, Square, Trash2 } from "@lucide/svelte";
	...
	<Card.Content class="flex flex-col gap-4">
		{#if newRuleDomains !== null || editingRuleId !== null}
			...
			<div class="flex flex-col gap-3 rounded-lg border border-dashed p-4">
				...
				<div class="flex flex-col gap-2">{#each currentFields as _, index}<div class="flex gap-2"><Input placeholder="例如: google.com" bind:value={currentFields[index]} aria-invalid={currentInvalids.has(index)} />{#if currentFields.length > 2}<Button variant="ghost" size="icon" onclick={() => removeDomain(isNew, index)} aria-label="移除域名"><Trash2 /></Button>{/if}</div>{/each}</div>
		...
		<div class="flex max-h-[480px] flex-1 flex-col gap-3 overflow-y-auto pr-1">
			{#each rules as rule, index (rule.id)}
				<div class={cn("flex items-center justify-between gap-4 rounded-lg border bg-muted/30 p-4", rule.excluded && "opacity-60")}>
					<div class="flex min-w-0 flex-1 items-center gap-3"><button onclick={() => toggleRule(index)} class="shrink-0 text-muted-foreground hover:text-primary" aria-label={rule.excluded ? "启用规则" : "禁用规则"}>{#if rule.excluded}<Square />{:else}<CheckSquare class="text-primary" />{/if}</button><div class="flex min-w-0 flex-1 flex-wrap gap-1.5">{#each rule.domains as domain}<code class="rounded-md border bg-background px-2 py-0.5 text-xs font-semibold">{domain}</code>{/each}</div></div>
