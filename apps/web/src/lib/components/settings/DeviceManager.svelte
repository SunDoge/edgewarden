...
async function removeSelectedDevices() {
	if (!selectedIdList.length || !confirm(`移除选中的 ${selectedIdList.length} 台设备并撤销其会话？`)) return;
