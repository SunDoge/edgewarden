...
<Card.Root>
	...
	<Card.Content><Button variant="outline" onclick={() => open = true}><Fingerprint data-icon="inline-start" />管理安全密钥</Button></Card.Content>
</Card.Root>

<Dialog.Root bind:open><Dialog.Content><Dialog.Header><Dialog.Title>两步验证安全密钥</Dialog.Title><Dialog.Description>先用主密码验证身份，再添加或删除安全密钥。</Dialog.Description></Dialog.Header><Field.Group><Field.Field><Field.Label for="two-factor-passkey-password">当前主密码</Field.Label><Input id="two-factor-passkey-password" type="password" bind:value={password} autocomplete="current-password" /></Field.Field><Field.Field orientation="horizontal"><Button variant="outline" onclick={load} disabled={!password || busy === "load"}>读取设置</Button></Field.Field>{#if credentials.length}<div class="flex flex-col gap-2">{#each credentials as credential (credential.id)}<div class="flex items-center justify-between rounded-md border p-3"><span>{credential.name || "安全密钥"}</span><Button variant="ghost" size="icon-sm" onclick={() => deleteId = String(credential.id)} disabled={busy === `delete-${credential.id}`} aria-label="删除安全密钥"><Trash2 data-icon /></Button></div>{/each}</div>{/if}<Field.Field><Field.Label for="two-factor-passkey-name">新安全密钥名称</Field.Label><Input id="two-factor-passkey-name" bind:value={name} placeholder="例如：USB 安全密钥" /></Field.Field></Field.Group><Dialog.Footer><Button variant="outline" onclick={() => open = false}>关闭</Button><Button onclick={add} disabled={!password || busy === "create"}><Fingerprint data-icon="inline-start" />添加安全密钥</Button></Dialog.Footer></Dialog.Content></Dialog.Root>

<AlertDialog.Root open={deleteId !== null} onOpenChange={(value) => { if (!value) deleteId = null; }}><AlertDialog.Content><AlertDialog.Header><AlertDialog.Title>删除安全密钥</AlertDialog.Title><AlertDialog.Description>删除后，这把安全密钥将不能再用于两步验证。</AlertDialog.Description></AlertDialog.Header><AlertDialog.Footer><AlertDialog.Cancel>取消</AlertDialog.Cancel><AlertDialog.Action class="bg-destructive text-destructive-foreground hover:bg-destructive/90" onclick={() => deleteId && remove(deleteId)}>确认删除</AlertDialog.Action></AlertDialog.Footer></AlertDialog.Content></AlertDialog.Root>
