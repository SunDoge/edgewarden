...
async function disable() {
	if (!password || !confirm("关闭 YubiKey 两步验证？")) return;
	busy = "disable";
	try {
...
<Card.Root>
	...
	<Card.Content><Button variant="outline" onclick={() => open = true}><KeyRound />管理 YubiKey</Button></Card.Content>
</Card.Root>
...
<Dialog.Root bind:open>
	<Dialog.Content class="max-h-[90vh] overflow-y-auto">
		...
		<Field.Group>
			...
			{#if settings}<div class="rounded-md border p-3 text-sm"><div>状态：{settings.enabled ? "已启用" : "未启用"}</div><div>已登记：{(settings.keys ?? []).join("、") || "无"}</div><div>Yubico 验证：{settings.configured ? "已配置" : "未配置"}</div></div>{/if}
			...
			{#if isAdmin}<div class="space-y-3 rounded-md border p-3"><div class="font-medium">Yubico 验证凭据（管理员）</div><Field.Field><Field.Label for="yubico-client-id">Client ID</Field.Label><Input id="yubico-client-id" bind:value={clientId} inputmode="numeric" /></Field.Field><Field.Field><Field.Label for="yubico-secret">Secret Key</Field.Label><Input id="yubico-secret" type="password" bind:value={secretKey} autocomplete="new-password" /></Field.Field><Button variant="outline" onclick={saveConfig} disabled={!password || !clientId || !secretKey || busy === "config"}>加密保存验证凭据</Button></div>{/if}
		</Field.Group>
		<Dialog.Footer><Button variant="destructive" onclick={disable} disabled={!settings?.enabled || busy === "disable"}>关闭 YubiKey</Button><Button onclick={save} disabled={!password || !otps.trim() || busy === "save"}>验证并保存</Button></Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
