...
describe("organization key envelopes", () => {
  it("wraps each organization key to an account RSA key", async () => {
    ...
    const accountPublic = bytesToBase64(
      new Uint8Array(
        await crypto.subtle.exportKey("spki", accountPair.publicKey),
      ),
    );
    const accountPrivate = new Uint8Array(
      await crypto.subtle.exportKey("pkcs8", accountPair.privateKey),
    );
    const encKey = crypto.getRandomValues(new Uint8Array(32));
    const macKey = crypto.getRandomValues(new Uint8Array(32));
    const importedPrivate = await importAccountPrivateKey(
      await encryptBw(accountPrivate, encKey, macKey),
      encKey,
      macKey,
    );
    ...
    const unwrapped = await unwrapOrganizationKey(
      materials.wrappedMemberKey,
      importedPrivate,
    );
