...
export const CLIENT_PREFERENCES_CHANGED_EVENT = "edgewarden:client-preferences-changed";
...
function isPreferences(value: unknown): value is ClientPreferences {
	...
	return ["system", "light", "dark"].includes(String(item.theme))
		&& [0, 1, 5, 15, 30].includes(Number(item.lockTimeoutMinutes))
		&& ["lock", "logout"].includes(String(item.sessionTimeoutAction));
}

export function loadClientPreferences(storage: Pick<Storage, "getItem"> = localStorage): ClientPreferences {
	try {
...
export function saveClientPreferences(value: ClientPreferences, storage: Pick<Storage, "setItem"> = localStorage): void {
...
export function applyThemePreference(preference: ThemePreference, root: HTMLElement = document.documentElement): void {
	...
	root.style.colorScheme = resolveDarkTheme(preference, systemDark) ? "dark" : "light";
}
