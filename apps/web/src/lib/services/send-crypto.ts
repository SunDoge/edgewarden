...
export async function decryptOwnedSend(
	...
	const result: DecryptedSend = { ...send, _sendKeys: keys, shareKey: encodeSendShareKey(raw) };
	...
	if (send.type === 0 && send.text) {
		const encryptedText = typeof send.text === "string" ? send.text : send.text.text;
		result.text = { ...send.text, text: await decryptText(encryptedText, keys) };
	}
	if (send.type === 1 && send.file) {
		result.file = { ...send.file, fileName: await decryptText(send.file.fileName, keys) };
	}
	return result;
}
...
export async function encryptSendMetadata(
	...
	keys: SendKeys,
): Promise<{ name: string; notes: string | null; text?: { text: string; hidden: false } }> {
	const result: { name: string; notes: string | null; text?: { text: string; hidden: false } } = {
		name: await encryptBw(utf8(input.name.trim()), keys.enc, keys.mac),
		notes: input.notes?.trim() ? await encryptBw(utf8(input.notes.trim()), keys.enc, keys.mac) : null,
	};
	if (input.text !== undefined) result.text = { text: await encryptBw(utf8(input.text), keys.enc, keys.mac), hidden: false };
	return result;
}

export async function wrapSendKey(keys: SendKeys, userEncKey: Uint8Array, userMacKey: Uint8Array): Promise<string> {
	return encryptBw(keys.raw, userEncKey, userMacKey);
}

export async function decryptPublicSend(send: Record<string, any>, keys: SendKeys): Promise<Record<string, any>> {
	const result = { ...send };
	result.name = send.name ? await decryptText(send.name, keys) : "";
	if (send.type === 0 && send.text) {
		result.text = await decryptText(typeof send.text === "string" ? send.text : send.text.text, keys);
