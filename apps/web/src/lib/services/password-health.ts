...
export async function inspectPasswordHealth(
	...
	let checked = 0;
	await Promise.all(
		Array.from({ length: Math.min(5, hashes.length) }, async () => {
			while (next < hashes.length) {
				const hash = hashes[next++];
				try {
					exposure.set(hash, await checkPasswordHash(hash, fetchImpl, signal));
				} catch (error) {
					if (signal?.aborted) throw error;
					exposure.set(hash, null);
				}
	...
	return {
		...
		items: all
			...
			.sort(
				(a, b) =>
					Number(b.exposedCount ?? 0) - Number(a.exposedCount ?? 0) ||
					b.reusedCount - a.reusedCount ||
