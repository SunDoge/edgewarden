...
import { encryptVaultKeyForAuthRequest, publicKeyFingerprint } from "./auth-requests";
...
describe("auth request cryptography", () => {
	it("encrypts exactly the current 64-byte vault key for the requester", async () => {
		...
		const wrapped = await encryptVaultKeyForAuthRequest(bytesToBase64(spki), enc, mac);
		expect(wrapped.startsWith("4.")).toBe(true);
		const decrypted = new Uint8Array(await crypto.subtle.decrypt({ name: "RSA-OAEP" }, pair.privateKey, toBufferSource(base64ToBytes(wrapped.slice(2)))));
	...
	it("produces a stable account-bound public-key fingerprint", async () => {
		...
		expect(await publicKeyFingerprint("user@example.com", publicKey)).toBe(first);
		expect(first).toMatch(/^[0-9a-f]{8}(?:-[0-9a-f]{8}){4}$/);
		expect(await publicKeyFingerprint("other@example.com", publicKey)).not.toBe(first);
	});

	it("rejects malformed or incorrectly sized key material", async () => {
		await expect(encryptVaultKeyForAuthRequest("bad", new Uint8Array(31), new Uint8Array(32))).rejects.toThrow("保险库密钥无效");
	});
});
