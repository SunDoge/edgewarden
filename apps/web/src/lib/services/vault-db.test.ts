...
function snapshot(accountId: string) {
	return {
		profile: {
			...
			key: "2.encrypted-profile-key",
			kdfIterations: 600_000,
		} as any,
		ciphers: [
			{ id: `${accountId}-cipher`, name: "2.encrypted-cipher-name" },
		] as any,
		folders: [
			{ id: `${accountId}-folder`, name: "2.encrypted-folder-name" },
		] as any,
		collections: [
			{
				id: `${accountId}-collection`,
				organizationId: `${accountId}-org`,
