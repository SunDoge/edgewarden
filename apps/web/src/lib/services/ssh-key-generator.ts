...
function required(jwk: JsonWebKey, property: keyof JsonWebKey): Uint8Array {
	...
	if (typeof value !== "string" || !value) throw new Error(`生成的密钥缺少 ${String(property)}`);
...
function privateEnvelope(publicBlob: Uint8Array, fields: Uint8Array[], comment: string): string {
	const check = crypto.getRandomValues(new Uint32Array(1))[0];
	let block = concat(uint32(check), uint32(check), ...fields, sshString(comment));
	const paddingLength = 8 - (block.length % 8);
	block = concat(block, Uint8Array.from({ length: paddingLength }, (_, index) => index + 1));
	const envelope = concat(encoder.encode("openssh-key-v1\0"), sshString("none"), sshString("none"), sshString(new Uint8Array()), uint32(1), sshString(publicBlob), sshString(block));
...
export async function computeSshFingerprint(publicKey: string): Promise<string> {
	...
	const blob = Uint8Array.from(atob(encoded), (character) => character.charCodeAt(0));
...
async function generateEd25519(comment: string): Promise<GeneratedSshKey> {
	...
	return { type: "ED25519", bits: 256, publicKey, privateKey: privateEnvelope(publicBlob, [sshString("ssh-ed25519"), sshString(publicBytes), sshString(concat(seed, publicBytes))], comment), fingerprint: await computeSshFingerprint(publicKey) };
}

async function generateRsa(length: 2048 | 3072 | 4096, comment: string): Promise<GeneratedSshKey> {
	const pair = await crypto.subtle.generateKey({ name: "RSASSA-PKCS1-v1_5", modulusLength: length, publicExponent: new Uint8Array([1, 0, 1]), hash: "SHA-256" }, true, ["sign", "verify"]) as CryptoKeyPair;
	const jwk = await crypto.subtle.exportKey("jwk", pair.privateKey);
	const [n, e, d, qi, p, q] = ["n", "e", "d", "qi", "p", "q"].map((key) => required(jwk, key as keyof JsonWebKey));
	const publicBlob = concat(sshString("ssh-rsa"), mpint(e), mpint(n));
	const publicKey = `ssh-rsa ${base64(publicBlob)}${comment ? ` ${comment}` : ""}`;
	return { type: "RSA", bits: length, publicKey, privateKey: privateEnvelope(publicBlob, [sshString("ssh-rsa"), mpint(n), mpint(e), mpint(d), mpint(qi), mpint(p), mpint(q)], comment), fingerprint: await computeSshFingerprint(publicKey) };
}
