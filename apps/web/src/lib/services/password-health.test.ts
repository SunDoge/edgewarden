...
import { checkPasswordHash, inspectPasswordHealth, isWeakPassword, parsePwnedRange, sha1Password } from "./password-health";
...
describe("password health", () => {
	it("uses SHA-1 k-anonymity and never sends the password", async () => {
		...
		let requestedUrl = "";
		const fetcher = vi.fn(async (url: string | URL | Request) => { requestedUrl = String(url); return new Response(`${hash.slice(5)}:42\n`); });
		expect(await checkPasswordHash(hash, fetcher as typeof fetch)).toBe(42);
		expect(requestedUrl).toBe(`https://api.pwnedpasswords.com/range/${hash.slice(0, 5)}`);
	...
	it("detects weak, reused, exposed and unavailable passwords", async () => {
		const fetcher = vi.fn(async (url: string | URL | Request) => new Response(String(url).endsWith("5BAA6") ? "1E4C9B93F3F0682250B6CF8331B7EE68FD8:100\n" : "", { status: 200 }));
		...
		] as any, fetcher as typeof fetch);
		expect(report).toMatchObject({ eligibleCount: 2, exposedCount: 2, reusedCount: 2, weakCount: 2, unavailableCount: 0 });
	...
	it("reports progress and excludes organization entries whose passwords must stay hidden", async () => {
		...
		const report = await inspectPasswordHealth([
			...
			{ id: "hidden", type: 1, login: { password: "password" }, hidePasswords: true },
		] as any, async () => new Response("1E4C9B93F3F0682250B6CF8331B7EE68FD8:1"), undefined, (checked, total) => progress.push([checked, total]));
