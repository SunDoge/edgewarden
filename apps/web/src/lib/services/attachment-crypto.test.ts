...
describe("attachment client-side encryption", () => {
	it("encrypts metadata and bytes before upload and round-trips locally", async () => {
		...
		const prepared = await prepareAttachment(
			cipher as any,
			new File(["top secret"], "payroll.txt"),
			userEnc,
			userMac,
		);
		expect(JSON.stringify(prepared.metadata)).not.toMatch(/payroll|top secret/);
		expect(new TextDecoder().decode(prepared.encryptedData)).not.toContain(
			"top secret",
		);
		const attachment = await decryptAttachmentMetadata(
			{
				id: "a",
				size: prepared.encryptedData.length,
				sizeName: "",
				object: "attachment",
				...prepared.metadata,
			} as any,
		...
		expect(
			new TextDecoder().decode(
				await decryptAttachmentFile(prepared.encryptedData, attachment._keys),
			),
		).toBe("top secret");
	});
	...
	it("rejects tampered encrypted content", async () => {
		...
		await expect(
			decryptAttachmentFile(prepared.encryptedData, attachment._keys),
		).rejects.toThrow();
	});

	it("sanitizes download file names", () =>
		expect(safeAttachmentFileName("../secret\0.txt")).toBe(".._secret_.txt"));
});
