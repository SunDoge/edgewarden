...
self.addEventListener("activate", (event) => {
	event.waitUntil(
		caches.keys()
			.then((keys) => Promise.all(keys.filter((key) => key.startsWith("edgewarden-shell-") && key !== CACHE).map((key) => caches.delete(key))))
...
self.addEventListener("fetch", (event) => {
	...
	if (!mayCacheRequest(request.method, request.url, self.location.origin)) return;
