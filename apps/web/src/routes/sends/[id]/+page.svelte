<script lang="ts">
  ...
  import { accessSendPublicApi, requestSendFileDownloadApi } from "$lib/services/api-sends";
  ...
  onMount(async () => {
    ...
    if (!hash) {
      error = "链接无效：缺少解密钥匙。请确保您复制了完整的分享链接（包括 '#' 后面的字符）。";
      loading = false;
      return;
    }
    ...
    <Card.Content class="flex flex-col gap-6">
      {#if loading}
        <Empty.Root
          ><Empty.Media variant="icon"><Spinner /></Empty.Media><Empty.Header
            ><Empty.Title>正在建立端到端解密通道</Empty.Title><Empty.Description
              >正在安全获取并解密分享内容。</Empty.Description
            ></Empty.Header
          ></Empty.Root
        >
      {:else if error && !passwordRequired}
        <Alert.Root variant="destructive"
          ><ShieldAlert /><Alert.Title>无法打开分享</Alert.Title><Alert.Description
      ...
      {:else if passwordRequired}
        ...
        >
          <Field.Group>
            <Alert.Root
              ><Lock /><Alert.Title>此安全分享已启用密码保护</Alert.Title><Alert.Description
                >请输入发送者设置的访问密码以继续解锁密文。</Alert.Description
              ></Alert.Root
            >

            {#if error}
              <Alert.Root variant="destructive"
                ><ShieldAlert /><Alert.Description>{error}</Alert.Description></Alert.Root
              >
            {/if}
            ...
            <Button type="submit" class="w-full font-semibold">验证密码并解锁</Button>
          </Field.Group>
        </form>
      {:else if sendData}
        <div class="flex flex-col gap-4">
          <Alert.Root
            ><LockOpen /><Alert.Title>解密成功</Alert.Title><Alert.Description>
          ...
          {#if sendData.type === 0}
            ...
            <div class="flex flex-col gap-2">
              ...
              <pre
                class="w-full select-all whitespace-pre-wrap break-all rounded-xl border bg-muted/50 p-4 font-mono text-sm leading-relaxed">{decryptedText}</pre>
            </div>
          {:else if sendData.type === 1}
            <!-- File Send payload -->
            <div
              class="flex flex-col items-center gap-4 rounded-2xl border bg-muted/30 p-5 text-center"
            >
              <div
