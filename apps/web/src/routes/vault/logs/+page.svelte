...
<main class="min-h-screen bg-muted/30 p-4 md:p-6"><div class="mx-auto flex max-w-7xl flex-col gap-6">
	<header class="flex items-start justify-between gap-2 sm:items-center"><div class="flex min-w-0 items-start gap-2 sm:items-center sm:gap-3"><Button variant="ghost" size="icon" class="shrink-0" onclick={() => goto("/vault")} aria-label="返回保险库"><ArrowLeft /></Button><div class="min-w-0"><h1 class="text-xl font-bold sm:text-2xl">审计日志</h1><p class="text-sm text-muted-foreground">登录、安全与管理操作记录；元数据会在服务端去除密钥和密码。</p></div></div><Button variant="outline" size="icon" class="shrink-0 sm:hidden" onclick={() => load()} disabled={busy} aria-label="刷新"><RefreshCw /></Button><Button variant="outline" class="hidden shrink-0 sm:inline-flex" onclick={() => load()} disabled={busy}><RefreshCw data-icon="inline-start" />刷新</Button></header>
