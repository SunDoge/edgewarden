		...
		<Card.Root>
			...
			<Card.Content>
				<Field.Group>
					...
					{:else if mode === "email"}
						<Field.Field><Field.Label>别名类型</Field.Label><Select.Root type="single" value={aliasMode} onValueChange={(value) => aliasMode = value as typeof aliasMode}><Select.Trigger class="w-full">{match(aliasMode).with("plus", () => "Plus Addressing").with("catchall", () => "Catch-all 域名").otherwise(() => "子域名")}</Select.Trigger><Select.Content><Select.Group><Select.Item value="plus">Plus Addressing</Select.Item><Select.Item value="catchall">Catch-all 域名</Select.Item><Select.Item value="subdomain">子域名</Select.Item></Select.Group></Select.Content></Select.Root></Field.Field>
