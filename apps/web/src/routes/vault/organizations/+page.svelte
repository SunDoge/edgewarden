<script lang="ts">
	...
	import { createOrganizationApi, createOrganizationCollectionApi, deleteOrganizationApi, deleteOrganizationCollectionApi, deriveAccountPasswordHash, getOrganizationInviteeApi, inviteOrganizationMemberApi, listOrganizationCollectionsApi, listOrganizationMembersApi, listOrganizationsApi, removeOrganizationMemberApi, updateOrganizationApi, updateOrganizationCollectionApi, updateOrganizationMemberApi } from "$lib/services/api";
	import { createOrganizationMaterials, wrapOrganizationKey } from "$lib/services/organization-crypto";
	import { encryptStr } from "$lib/services/crypto";
	import { getOrganizationKey, syncVaultData, vault } from "$lib/stores/vault.svelte";
	...
	import { ArrowLeft, Building2, Pencil, Plus, Trash2, UserPlus } from "@lucide/svelte";
	...
	let memberCollectionAccess = $state<Record<string, { selected: boolean; readOnly: boolean; hidePasswords: boolean }>>({});
	...
	async function load() {
		...
		try {
			...
			if (selected) selected = organizations.find((item) => item.id === selected.id) ?? organizations[0] ?? null;
			else selected = organizations[0] ?? null;
			await loadSelected();
		} catch (value) { error = value instanceof Error ? value.message : "加载组织失败"; }
		finally { loading = false; }
	}

	async function loadSelected() {
		if (!selected) { members = []; collections = []; return; }
		const tasks: Promise<any>[] = [listOrganizationCollectionsApi(selected.id)];
		if (["owner", "admin", "manager"].includes(selected.role)) tasks.push(listOrganizationMembersApi(selected.id));
		const results = await Promise.all(tasks);
		collections = results[0].data ?? [];
		members = results[1]?.data ?? [];
	}

	onMount(async () => { if (!vault.isUnlocked) return goto("/vault/unlock"); await load(); });

	async function createOrganization() {
		if (!vault.profile?.publicKey || !organizationName.trim() || !initialCollectionName.trim()) return;
		busy = "create";
		try {
			const materials = await createOrganizationMaterials(vault.profile.publicKey, initialCollectionName.trim());
			await createOrganizationApi({ name: organizationName.trim(), collectionName: materials.encryptedCollectionName, key: materials.wrappedMemberKey, publicKey: materials.publicKey, encryptedPrivateKey: materials.encryptedPrivateKey });
			createOpen = false;
			organizationName = "";
			await syncVaultData();
			await load();
		} catch (value) { error = value instanceof Error ? value.message : "创建组织失败"; }
		finally { busy = ""; }
	}
	...
	async function inviteMember() {
		...
		if (!organizationKey) { error = "组织密钥不可用，请重新同步并解锁"; return; }
		busy = "invite";
		try {
			const invitee = await getOrganizationInviteeApi(selected.id, inviteEmail.trim());
			if (!inviteAccessAll && !inviteCollectionIds.length) throw new Error("受限成员至少需要选择一个集合");
			await inviteOrganizationMemberApi(selected.id, { email: invitee.email, role: inviteRole, accessAll: inviteAccessAll, collections: inviteAccessAll ? [] : inviteCollectionIds.map((id) => ({ id, readOnly: false, hidePasswords: false })), key: await wrapOrganizationKey(organizationKey, invitee.publicKey) });
			inviteEmail = "";
			inviteAccessAll = true;
			inviteCollectionIds = [];
			members = (await listOrganizationMembersApi(selected.id)).data ?? [];
		} catch (value) { error = value instanceof Error ? value.message : "添加成员失败"; }
		finally { busy = ""; }
	}
	...
	function editMember(member: any) {
		...
		memberCollectionAccess = Object.fromEntries(vault.collections.filter((item) => item.organizationId === selected?.id).map((collection) => {
			const current = (member.collections ?? []).find((item: any) => item.id === collection.id);
			return [collection.id, { selected: Boolean(current), readOnly: Boolean(current?.readOnly), hidePasswords: Boolean(current?.hidePasswords) }];
		}));
	}

	async function saveMember() {
		if (!selected || !editingMember) return;
		const selectedCollections = Object.entries(memberCollectionAccess).filter(([, access]) => access.selected).map(([id, access]) => ({ id, readOnly: access.readOnly, hidePasswords: access.hidePasswords }));
		if (!memberAccessAll && !selectedCollections.length) { error = "受限成员至少需要选择一个集合"; return; }
		busy = "member";
		try {
			await updateOrganizationMemberApi(selected.id, editingMember.id, { role: memberRole, accessAll: memberAccessAll, collections: memberAccessAll ? [] : selectedCollections });
			editingMember = null;
			members = (await listOrganizationMembersApi(selected.id)).data ?? [];
			await syncVaultData();
		} catch (value) { error = value instanceof Error ? value.message : "更新成员失败"; }
		finally { busy = ""; }
	}
	...
	async function addCollection() {
		...
		busy = "collection";
		try {
			await createOrganizationCollectionApi(selected.id, await encryptStr(collectionName.trim(), key.encKey, key.macKey));
			collectionName = "";
			await syncVaultData();
			collections = vault.collections.filter((item) => item.organizationId === selected.id);
		} catch (value) { error = value instanceof Error ? value.message : "创建集合失败"; }
		finally { busy = ""; }
	}
	...
	async function renameOrganization() {
		...
		if (!name) return;
		try { await updateOrganizationApi(selected.id, name); await syncVaultData(); await load(); }
		catch (value) { error = value instanceof Error ? value.message : "重命名组织失败"; }
	}
	...
	async function renameCollection(collection: any) {
		...
		const key = getOrganizationKey(selected.id);
		if (!name || !key) return;
		try { await updateOrganizationCollectionApi(selected.id, collection.id, await encryptStr(name, key.encKey, key.macKey)); await syncVaultData(); collections = vault.collections.filter((item) => item.organizationId === selected.id); }
		catch (value) { error = value instanceof Error ? value.message : "重命名集合失败"; }
	}
	...
	async function removeCollection(collection: any) {
		if (!selected || !confirm(`删除集合 ${collection.name}？集合内条目必须先移动或删除。`)) return;
	...
	async function removeOrganization() {
		...
		busy = "delete-org";
		try {
			await deleteOrganizationApi(selected.id, await deriveAccountPasswordHash(vault.profile.email, deletePassword));
			deletePassword = "";
			selected = null;
			await syncVaultData();
			await load();
		} catch (value) { error = value instanceof Error ? value.message : "删除组织失败"; }
		finally { busy = ""; }
	}
</script>
		...
		<div class="grid gap-6 md:grid-cols-[16rem_1fr]">
			...
			{#if ["owner", "admin", "manager"].includes(selected.role)}
				<Card.Root><Card.Header><Card.Title>成员</Card.Title></Card.Header><Card.Content class="space-y-4">
					...
					<Table.Root><Table.Header><Table.Row><Table.Head>邮箱</Table.Head><Table.Head>角色</Table.Head><Table.Head>范围</Table.Head><Table.Head class="text-right">操作</Table.Head></Table.Row></Table.Header><Table.Body>{#each members as member (member.id)}<Table.Row><Table.Cell>{member.email}</Table.Cell><Table.Cell>{member.role}</Table.Cell><Table.Cell>{member.accessAll ? "全部" : `${member.collections?.length ?? 0} 个集合`}</Table.Cell><Table.Cell class="text-right">{#if member.role !== "owner"}<Button variant="ghost" size="icon-sm" onclick={() => editMember(member)} aria-label="编辑成员"><Pencil /></Button><Button variant="ghost" size="icon-sm" onclick={() => removeMember(member)} aria-label="移除成员"><Trash2 /></Button>{/if}</Table.Cell></Table.Row>{/each}</Table.Body></Table.Root>
