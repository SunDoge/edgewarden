<script lang="ts">
	...
	import { createAdminInviteApi, deleteAdminInviteApi, deleteAdminUserApi, deriveAccountPasswordHash, listAdminInvitesApi, listAdminUsersApi, setAdminUserStatusApi } from "$lib/services/api";
	import { vault } from "$lib/stores/vault.svelte";
	import { ArrowLeft, Copy, RefreshCw, ShieldAlert, Trash2, UserRoundCog } from "@lucide/svelte";
	...
	async function refresh() {
		busy = "refresh";
		error = null;
		try { [users, invites] = await Promise.all([listAdminUsersApi().then((r) => r.data), listAdminInvitesApi().then((r) => r.data)]); }
	...
	async function run(key: string, operation: (hash: string) => Promise<unknown>) {
		busy = key;
		error = null;
		try { await operation(await passwordHash()); masterPassword = ""; await refresh(); }
		catch (reason) { error = reason instanceof Error ? reason.message : String(reason); }
		finally { busy = null; }
	}
	...
	onMount(() => { if (vault.profile?.role !== "admin") void goto("/vault"); else void refresh(); });
</script>
