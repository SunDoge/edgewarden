...
<div class="flex min-h-screen items-center justify-center bg-muted/30 p-4">
  ...
  <Card.Root class="w-full max-w-sm">
    ...
    <Card.Header class="items-center gap-2 text-center">
      <div
        class="mb-4 flex size-14 items-center justify-center rounded-xl bg-primary text-primary-foreground"
      >
    ...
    <Card.Content class="flex flex-col gap-4">
      {#if !hasCache}
        ...
        <Alert.Root
          ><WifiOff /><Alert.Title>需要联网登录</Alert.Title><Alert.Description
            >首次使用请先联网登录，之后才可离线解锁。</Alert.Description
          ></Alert.Root
        >
      ...
      {:else}
        ...
        {#if error}
          <Alert.Root variant="destructive"
            ><Alert.Title>解锁失败</Alert.Title><Alert.Description>{error}</Alert.Description
            ></Alert.Root
          >
        {/if}
