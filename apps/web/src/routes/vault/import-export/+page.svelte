...
<VaultPageShell title="数据导入与导出" description="在 Edgewarden 与兼容格式之间安全迁移保险库数据。" width="default">
		...
		{#if successMsg}
			<Alert.Root><CheckCircle /><Alert.Title>操作成功</Alert.Title><Alert.Description>{successMsg}</Alert.Description></Alert.Root>
		{/if}
		...
		<div class="grid gap-4 md:grid-cols-2 md:gap-6">
			...
			<Card.Root>
				...
				<Card.Content><Field.Group>
					...
					<Field.Field><Field.Label>文件格式</Field.Label><Select.Root type="single" value={importFormat} onValueChange={(value) => { importFormat = value as "json" | "csv"; resetImportSelection(); }}><Select.Trigger class="w-full">{importFormat === "json" ? "Bitwarden JSON（密码保护或未加密）" : "Bitwarden CSV"}</Select.Trigger><Select.Content><Select.Group><Select.Item value="json">Bitwarden JSON（密码保护或未加密）</Select.Item><Select.Item value="csv">Bitwarden CSV</Select.Item></Select.Group></Select.Content></Select.Root></Field.Field>
			...
			<Card.Root>
				...
				<Card.Content><Field.Group>
					<Field.Field><Field.Label>导出格式</Field.Label><Select.Root type="single" value={exportFormat} onValueChange={(value) => exportFormat = value as "json" | "csv"}><Select.Trigger class="w-full">{exportFormat === "json" ? "Bitwarden JSON（完整）" : "Bitwarden CSV（登录与笔记）"}</Select.Trigger><Select.Content><Select.Group><Select.Item value="json">Bitwarden JSON（完整）</Select.Item><Select.Item value="csv">Bitwarden CSV（登录与笔记）</Select.Item></Select.Group></Select.Content></Select.Root></Field.Field>
					<Alert.Root><ShieldAlert /><Alert.Title>安全警告</Alert.Title><Alert.Description>导出的备份文件包含明文用户名、密码、笔记以及支付卡片。请勿发送给他人，用完后立即删除或存放于安全位置。</Alert.Description></Alert.Root>
					<Separator />
...
<AlertDialog.Root bind:open={exportConfirmOpen}>
	<AlertDialog.Content>
		<AlertDialog.Header><AlertDialog.Title>确认导出明文保险库</AlertDialog.Title><AlertDialog.Description>导出的文件不会加密，任何获得该文件的人都能读取其中的密码。请确认你能安全保存并在使用后删除它。</AlertDialog.Description></AlertDialog.Header>
		<AlertDialog.Footer><AlertDialog.Cancel>取消</AlertDialog.Cancel><AlertDialog.Action class="bg-destructive text-destructive-foreground hover:bg-destructive/90" onclick={handleExport}>确认导出</AlertDialog.Action></AlertDialog.Footer>
	</AlertDialog.Content>
</AlertDialog.Root>
