<script lang="ts">
  ...
  import { fetchSendsApi, deleteSendApi, deleteSendsApi } from "$lib/services/api-sends";
  import { vault } from "$lib/stores/vault.svelte";
  import { decryptOwnedSend, type DecryptedSend } from "$lib/services/send-crypto";
  import { saveOwnedSend } from "$lib/services/send-actions";
  import { createSendEditorDraft, sendToEditorDraft } from "$lib/services/send-editor";
  ...
  let errorMsg = $state("");
  let deleteTarget = $state<{ kind: "single"; id: string } | { kind: "bulk" } | null>(null);
...
<VaultPageShell
  title="Send 传输中心"
  description="创建和管理端到端加密的临时分享。"
  width="full"
  fill
>
  {#snippet actions()}
    <Button variant="ghost" size="sm" onclick={loadSends} disabled={loading} aria-label="刷新 Send">
  ...
  <div class="relative flex flex-1 overflow-hidden">
    ...
    <section
      class="{mobileDetailOpen
        ? 'hidden md:flex'
        : 'flex'} min-w-0 flex-1 flex-col overflow-hidden border-r border-border bg-background"
    >
      {#if errorMsg}<div class="p-3">
          <Alert.Root variant="destructive"
            ><Alert.Title>操作失败</Alert.Title><Alert.Description>{errorMsg}</Alert.Description
            ></Alert.Root
          >
        </div>{/if}
      ...
      <div class="flex items-center gap-2 border-b px-4 py-2">
        <ToggleGroup.Root
          ...
          value={typeFilter}
          onValueChange={(value) => {
            if (value) typeFilter = value as typeof typeFilter;
          }}
          >{#each [["all", "全部"], ["text", "文本"], ["file", "文件"]] as option}<ToggleGroup.Item
              value={option[0]}>{option[1]}</ToggleGroup.Item
            >{/each}</ToggleGroup.Root
        >
        <span class="flex-1"></span>
        {#if selectedIdList.length}<Button
            size="sm"
            variant="destructive"
            onclick={() => (deleteTarget = { kind: "bulk" })}
            ><Trash2 data-icon="inline-start" />删除 {selectedIdList.length} 项</Button
          >{/if}
      </div>
      ...
      <div class="flex-1 divide-y overflow-y-auto">
        {#if loading && sends.length === 0}
          <Empty.Root
            ><Empty.Header
              ><Empty.Media><Spinner class="size-8" /></Empty.Media><Empty.Title
                >正在加载 Send</Empty.Title
              ><Empty.Description>正在解密并整理你的安全分享。</Empty.Description></Empty.Header
            ></Empty.Root
          >
        {:else if filteredSends.length === 0}
          <Empty.Root
            ><Empty.Header
              ><Empty.Media variant="icon"><Share2 /></Empty.Media><Empty.Title
                >暂无分享文件或文本</Empty.Title
              ><Empty.Description>创建安全链接，随时随地发送加密文本和文件。</Empty.Description
              ></Empty.Header
            ><Empty.Content
              ><Button onclick={startCreate}><Plus data-icon="inline-start" />新建 Send</Button
              ></Empty.Content
            ></Empty.Root
          >
        {:else}
          {#each filteredSends as send}
            <div
              ...
              tabindex="0"
              class={cn(
                "flex w-full items-center justify-between border-l-2 border-transparent p-4 text-left transition-colors hover:bg-muted/50",
                selectedSend?.id === send.id && "border-primary bg-muted/60",
              )}
              onclick={() => selectSend(send)}
              onkeydown={(event) => {
                if (event.key === "Enter" || event.key === " ") selectSend(send);
              }}
            >
              <div class="flex items-center gap-3.5 min-w-0 flex-1">
                <Checkbox
                  checked={Boolean(selectedIds[send.id])}
                  onclick={(event) => event.stopPropagation()}
                  onCheckedChange={(checked) =>
                    (selectedIds = { ...selectedIds, [send.id]: checked })}
                  aria-label={`选择 ${send.name}`}
                />
                <div
    ...
    >
      <div class="mb-4 md:hidden">
        <Button
          ...
          size="sm"
          onclick={() => {
            if (isCreating || isEditing) cancelEdit();
            else mobileDetailOpen = false;
          }}><ArrowLeft />返回列表</Button
        >
      </div>
      ...
      {:else}
        <Empty.Root class="h-full"
          ><Empty.Header
            ><Empty.Media variant="icon"><Share2 /></Empty.Media><Empty.Title
              >选择一个 Send 查看传输详情</Empty.Title
            ><Empty.Description
              >您创建的任何阅后即焚内容将在此列出，并附带管理统计。</Empty.Description
            ></Empty.Header
          ></Empty.Root
        >
      {/if}
...
>
  <AlertDialog.Content
    ><AlertDialog.Header
      ><AlertDialog.Title>确认删除 Send</AlertDialog.Title><AlertDialog.Description
        >{deleteTarget?.kind === "bulk"
          ? `将永久删除选中的 ${selectedIdList.length} 个 Send。`
          : "此 Send 将被永久删除。"} 此操作无法撤销。</AlertDialog.Description
      ></AlertDialog.Header
    ><AlertDialog.Footer
      ><AlertDialog.Cancel>取消</AlertDialog.Cancel><AlertDialog.Action
        class="bg-destructive text-destructive-foreground hover:bg-destructive/90"
        onclick={confirmDelete}>确认删除</AlertDialog.Action
      ></AlertDialog.Footer
    ></AlertDialog.Content
  >
</AlertDialog.Root>
