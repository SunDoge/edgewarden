...
if (!server || !email || !password) {
	throw new Error("BW_SERVER, BW_EMAIL, and BW_PASSWORD must target a disposable compatibility-test account");
}
...
async function bw(args: string[], options: BwOptions = {}): Promise<string> {
	...
	if (exitCode !== 0) {
		throw new Error(`bw ${args[0] ?? "command"} failed: ${stderr.trim() || stdout.trim() || `exit ${exitCode}`}`);
	}
...
async function cleanup(): Promise<void> {
	if (session && itemId) await bw(["delete", "item", itemId, "--permanent"], { session, quiet: true }).catch(() => {});
	if (session && folderId) await bw(["delete", "folder", folderId], { session, quiet: true }).catch(() => {});
...
try {
	...
	session = await bw(["login", email, "--passwordenv", "BW_PASSWORD", "--raw"], { quiet: true });
	await bw(["sync"], { session, quiet: true });

	const folder = JSON.parse(await bw([
		"create",
		"folder",
		encode({ name: `Edgewarden CLI smoke ${Date.now()}` }),
	], { session, quiet: true })) as { id: string };
	folderId = folder.id;

	const item = JSON.parse(await bw([
		"create",
		"item",
		encode({
			type: 1,
	...
	await bw(["sync"], { session, quiet: true });
	const roundTripped = JSON.parse(await bw(["get", "item", itemId], { session, quiet: true })) as {
		name?: string;
		login?: { username?: string };
	};
	if (roundTripped.name !== "Edgewarden CLI smoke login" || roundTripped.login?.username !== "smoke-user") {
	...
	folderId = "";
	console.log("Bitwarden CLI login, sync, create, read, and delete smoke test passed.");
} finally {
	await cleanup();
}
