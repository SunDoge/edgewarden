import { deriveMasterKey, deriveMasterPasswordHash } from "../apps/web/src/lib/services/crypto";
...
if (!origin || !email || !password) {
	throw new Error("EDGEWARDEN_SERVER, EDGEWARDEN_EMAIL, and EDGEWARDEN_PASSWORD must target a disposable integration-test account");
}
...
async function api(path: string, init: RequestInit = {}): Promise<Response> {
	...
	return fetch(new URL(path, origin), { ...init, headers, ...(tls ? { tls } : {}) } as RequestInit);
}
...
async function login(): Promise<void> {
	const prelogin = await json<{ kdfIterations: number }>(await api("/identity/accounts/prelogin/password", {
		method: "POST",
		headers: { "content-type": "application/json" },
		body: JSON.stringify({ email }),
	}));
	const masterKey = await deriveMasterKey(password!, email!, prelogin.kdfIterations);
	...
	const tokens = await json<{ access_token?: string; accessToken?: string }>(await api("/identity/connect/token", {
		method: "POST",
...
async function connectRealtime(): Promise<void> {
	const ticket = await json<{ token: string }>(await api("/api/notifications/token", { method: "POST" }));
...
function nextVaultRevision(): Promise<number> {
	return new Promise((resolve, reject) => {
		const timeout = setTimeout(() => reject(new Error("Realtime revision timeout")), 5_000);
		socket!.addEventListener("message", (event) => {
			try {
				const message = JSON.parse(String(event.data)) as { type?: string; revisionDate?: number };
...
async function verifyRealtime(): Promise<void> {
	...
	const folder = await json<{ id: string }>(await api("/api/folders", {
		method: "POST",
		headers: { "content-type": "application/json" },
...
async function verifyR2Attachment(): Promise<void> {
	const cipher = await json<{ id: string }>(await api("/api/ciphers", {
		method: "POST",
		headers: { "content-type": "application/json" },
		body: JSON.stringify({
			type: 1,
	...
	const attachment = await json<{ attachmentId: string; url: string }>(await api(`/api/ciphers/${cipherId}/attachment/v2`, {
		method: "POST",
		headers: { "content-type": "application/json" },
		body: JSON.stringify({ fileName: "encrypted-file-name", key: "encrypted-attachment-key", fileSize: bytes.byteLength }),
	}));
	...
	if (!uploaded.ok) throw new Error(`Attachment upload failed: ${uploaded.status} ${await uploaded.text()}`);
	const downloadedResponse = await api(`/api/ciphers/${cipherId}/attachment/${attachment.attachmentId}`);
	if (!downloadedResponse.ok) throw new Error(`Attachment download failed: ${downloadedResponse.status}`);
	const downloaded = new Uint8Array(await downloadedResponse.arrayBuffer());
	if (downloaded.length !== bytes.length || downloaded.some((value, index) => value !== bytes[index])) {
...
async function cleanup(): Promise<void> {
	...
	if (folderId) await api(`/api/folders/${folderId}`, { method: "DELETE" }).catch(() => null);
	if (cipherId) await api(`/api/ciphers/${cipherId}`, { method: "DELETE" }).catch(() => null);
}
...
try {
	...
	console.log("Durable Object WebSocket and R2 attachment integration smoke test passed.");
} finally {
	await cleanup();
}
